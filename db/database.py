...
class DB:
    ...
    async def connect_db(cls):
        ...
        try:
            ...
            if url:
                ...
                cls.con = await asyncpg.connect(database=Config.DATABASE, user=Config.USER,
        ...
        if not a[0][0]:
            try:
                await cls.con.execute(
                    ...
                    '(id serial primary key, login text not null unique,'
        ...
        if not a[0][0]:
            try:
                await cls.con.execute(
                    ...
                    "(id serial primary key, owner_id integer not null references user_data(id),"
                    " title text not null, description text,liked integer, unique(title,owner_id)) ;")
            except Exception as er:
                print(er)
                await cls.con.close()
                return False
        ...
        try:
            a = await cls.con.fetch("select exists(select * from information_schema.tables "
                                    "where table_name=$1)", 'liked_data')
        except Exception as er:
            print(er)
            await cls.con.close()
            return False
    ...
    async def add_new_user_db(cls, login: str, password: str, username: str):
        ...
        :return: True or False
        '''
        sql = "insert into user_data (login,password,username) values ($1,$2,$3) ;"
        try:
    ...
    async def get_user_by_login_db(cls, login: str):
        ...
        '''
        sql = "select * from user_data where login=$1"
        try:
    ...
    async def update_user_db(cls, password: str, username: str, id: int):
        ...
        '''
        if password is None:
            sql = "update user_data set username=$1 where id=$2"
            try:
            ...
            sql = "update user_data set password=$1, username=$2 where id=$3"
            try:
    ...
    async def get_posts_of_user(cls, owner_id: int):
        ...
        :return: list() or False
        '''
        sql = "select user_data.username,feed_data.id,user_data.id," \
              " feed_data.title ,feed_data.description,feed_data.liked " \
              "from feed_data inner join user_data " \
              "on user_data.id = feed_data.owner_id where owner_id=$1;"
        try:
    ...
    async def get_post(cls, feed_id: int):
        ...
        sql = "select user_data.username,feed_data.id,user_data.id," \
              " feed_data.title ,feed_data.description,feed_data.liked " \
              "from feed_data inner join user_data " \
              "on user_data.id = feed_data.owner_id where feed_data.id = $1 "
        try:
    ...
    async def get_random_n_posts(cls, number_of_posts: int):
        ...
        sql = "select user_data.username,feed_data.id,user_data.id, " \
              "feed_data.title ,feed_data.description,feed_data.liked " \
              "from feed_data inner join user_data " \
              "on user_data.id = feed_data.owner_id order by random() limit $1;"
        try:
    ...
    async def get_all_posts(cls):
        ...
        sql = "select user_data.username,feed_data.id,user_data.id," \
              " feed_data.title ,feed_data.description ,feed_data.liked " \
              "from feed_data inner join user_data on user_data.id = feed_data.owner_id"
        try:
